...
echo
echo "Creating ticket-topic in SNS"
echo "----------------------------"
awslocal sns create-topic --name ticket-topic
...
echo
echo "Subscribing $SQS_SPORT_QUEUE to ticket-topic with event_type=sport FilterPolicy"
echo "---------------------------------------------"
awslocal sns subscribe \
  --topic-arn arn:aws:sns:eu-west-1:000000000000:ticket-topic \
  --protocol sqs \
  --attributes '{"RawMessageDelivery":"true"}' \
  --notification-endpoint arn:aws:sqs:eu-west-1:000000000000:$SQS_SPORT_QUEUE \
  --attributes '{"RawMessageDelivery": "true", "FilterPolicy": "{\"event_type\": [\"sport\"]}"}'


echo
echo "Subscribing $SQS_MOVIE_QUEUE to ticket-topic with event_type=movie FilterPolicy"
echo "---------------------------------------------"
awslocal sns subscribe \
  --topic-arn arn:aws:sns:eu-west-1:000000000000:ticket-topic \
  --protocol sqs \
  --attributes '{"RawMessageDelivery":"true"}' \
  --notification-endpoint arn:aws:sqs:eu-west-1:000000000000:$SQS_MOVIE_QUEUE \
