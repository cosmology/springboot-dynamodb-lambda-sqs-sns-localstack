...
public class RandomTicketGenerator {
    ...
    private String determineEventType(String title) {
        ...
        } else if (title.matches(".*@.*(AMC|Cinema|Theatre|Film|IMAX).*") || title.toLowerCase().contains("movie")) {
            return "movie"; // Movie-related titles
        } else if (title.matches(".*@.*(Restaurant|Cafe|Bar|Grill).*") || title.toLowerCase().contains("restaurant")) {
    ...
    // Sports Events
        ...
        generateEvent("The Hunger Games: The Ballad of Songbirds and Snakes @ IMAX Theatre, San Francisco", 7),
        generateEvent("Wish @ AMC The Grove 14, Los Angeles", 8),
        generateEvent("Napoleon @ Vue West End Theatre, London", 9),
        generateEvent("Spider-Man: Across the Spider-Verse @ Pacific Theatre at the Americana, Glendale", 10),
